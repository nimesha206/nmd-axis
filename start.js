...
async function autoInstallDependencies() {
    ...
    //   ffmpeg    - audio/video encode, nmd_axis ක්‍රම 23-26
    //   python3   - yt-dlp + youtube-dl runtime
    //   yt-dlp    - nmd_axis.js ක්‍රම 1-16 (client variants 15)
    //
    // විකල්ප (නැතිනම් fallback ක්‍රම use කෙරේ):
    //   youtube-dl  - nmd_axis ක්‍රම 9-14 fallback
    //   curl/wget   - nmd_axis ක්‍රම 27-28, API download
    //   aria2c      - nmd_axis ක්‍රම 29 multi-thread
    //   sox         - nmd_axis ක්‍රම 30 audio convert
    ...
    const optionalSysDeps = {
        'curl':        'HTTP streaming + API calls (nmd_axis & lib/scraper 50+ methods)',
        'wget':        'direct file download fallback (nmd_axis method 29)',
        'git':         'version control',
        'spotifydl':   'Spotify track download (nmd_axis .spotify command)',
        'imagemagick': 'image processing / sticker creation',
        'ghostscript': 'PDF/document processing',
        'youtube-dl':  'YT download fallback (nmd_axis methods 9-14)',
        'aria2c':      'multi-thread download (nmd_axis method 29)',
        'sox':         'audio format conversion (nmd_axis method 30 fallback)',
