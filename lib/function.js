...
const { proto, areJidsSameUser, extractMessageContent, downloadContentFromMessage, getContentType, getDevice } = global._baileysModule || {};
...
// Runtime function එක සිංහලට පරිවර්තනය කරන ලදි
const runtime = function(seconds) {
	...
	var sDisplay = s > 0 ? s + (s == 1 ? " තත්පරය" : " තත්පර") : "";
...
async function updateSettings({ filePath, owner, author, apikey, botname, packname }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (apikey) content = content.replace(/(global\.APIKeys\s*=\s*\{[\s\S]*?'\https:\/\/api\.nima\.biz\.id'\s*:\s*')[^']*(')/, `$1${apikey}$2`);
...
// Installation error messages සිංහලට පරිවර්තනය කරන ලදි
function assertInstalled(cmd, name, code) {
	try {
		execSync(cmd, { stdio: 'ignore' });
	} catch (e) {
		console.error(chalk.redBright(`❌  ${name} ස්ථාපනය කර නැත.`) +`\nකරුණාකර එය මුලින්ම ස්ථාපනය කර නැවත උත්සාහ කරන්න.\n`);
		process.exit(code);
	}
}
